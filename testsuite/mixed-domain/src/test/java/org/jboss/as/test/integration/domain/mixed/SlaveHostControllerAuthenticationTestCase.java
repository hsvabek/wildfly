 ...
 */
public abstract class SlaveHostControllerAuthenticationTestCase {
    ...
    public static void before(Class<?> testClass) throws Exception {    
        ...
        MixedDomainTestSupport.copyDomainFile();
        
        Version.AsVersion version = MixedDomainTestSuite.getVersion(testClass);
        testSupport = MixedDomainTestSupport.create(testClass.getSimpleName(),
                version,
                "host-configs/host-master-no-local.xml",
                "host-configs/host-secrets.xml",
                true
                );
