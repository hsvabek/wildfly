 ...
 */
public abstract class DomainDeploymentOverlayCliTestCase {
    ...
    public static void before(Class<?> testClass) throws Exception {
        ...
        final URL overrideXmlUrl = DomainDeploymentOverlayCliTestCase.class.getClassLoader().getResource("cli/deployment-overlay/override.xml");
        ...
        final URL webXmlUrl = DomainDeploymentOverlayCliTestCase.class.getClassLoader().getResource("cli/deployment-overlay/web.xml");
        if(webXmlUrl == null) {
    ...
    public void setUp() throws Exception {
        client = support.getDomainMasterLifecycleUtil().createDomainClient();
        ctx = CLITestUtil.getCommandContext();
        //todo replace with proper fix coming with newer wildfly core.
        ctx.connectController(support.getDomainMasterConfiguration().getHostControllerManagementAddress(),support.getDomainMasterConfiguration().getHostControllerManagementPort());
    }
    ...
    public void testSimpleOverride() throws Exception {
        ...
        ctx.handle("deploy --server-groups=other-server-group " + war1.getAbsolutePath());
        ctx.handle("deploy --server-groups=other-server-group " + war2.getAbsolutePath());

        ctx.handle("deployment-overlay add --name=overlay-test --content=WEB-INF/web.xml=" + overrideXml.getAbsolutePath()
                + " --deployments=" + war1.getName() + " --server-groups=other-server-group");

        assertEquals("NON OVERRIDDEN", performHttpCall("slave", "server-one", "deployment0"));
        assertEquals("NON OVERRIDDEN", performHttpCall("slave", "server-one", "deployment1"));

        ctx.handle("deployment-overlay redeploy-affected --name=overlay-test");

        assertEquals("OVERRIDDEN", performHttpCall("slave", "server-one", "deployment0"));
        assertEquals("NON OVERRIDDEN", performHttpCall("slave", "server-one", "deployment1"));
    }
    ...
    public void testSimpleOverrideWithRedeployAffected() throws Exception {
        ...
        ctx.handle("deployment-overlay add --name=overlay-test --content=WEB-INF/web.xml=" + overrideXml.getAbsolutePath()
                + " --deployments=" + war1.getName() + " --server-groups=other-server-group --redeploy-affected");

//        assertEquals("OVERRIDDEN", performHttpCall("master", "server-one", "deployment0"));
//        assertEquals("NON OVERRIDDEN", performHttpCall("master", "server-one", "deployment1"));
        assertEquals("OVERRIDDEN", performHttpCall("slave", "server-one", "deployment0"));
        assertEquals("NON OVERRIDDEN", performHttpCall("slave", "server-one", "deployment1"));
    }
    ...
    public void testWildcardOverride() throws Exception {
        ...
        ctx.handle("deployment-overlay add --name=overlay-test --content=WEB-INF/web.xml=" + overrideXml.getAbsolutePath()
                + " --deployments=deployment*.war --server-groups=other-server-group --redeploy-affected");

        ctx.handle("deploy --server-groups=other-server-group " + war1.getAbsolutePath());
        ctx.handle("deploy --server-groups=other-server-group " + war2.getAbsolutePath());
        ctx.handle("deploy --server-groups=other-server-group " + war3.getAbsolutePath());

        assertEquals("OVERRIDDEN", performHttpCall("slave", "server-one", "deployment0"));
        assertEquals("OVERRIDDEN", performHttpCall("slave", "server-one", "deployment1"));
        assertEquals("NON OVERRIDDEN", performHttpCall("slave", "server-one", "another"));
    }
    ...
    public void testMultipleLinks() throws Exception {
        ...
        ctx.handle("deployment-overlay link --name=overlay-test --deployments=a*.war --server-groups=other-server-group");

        assertEquals("OVERRIDDEN", performHttpCall("slave", "server-one", "deployment0"));
        assertEquals("NON OVERRIDDEN", performHttpCall("slave", "server-one", "deployment1"));
        assertEquals("NON OVERRIDDEN", performHttpCall("slave", "server-one", "another"));

        ctx.handle("/server-group=other-server-group/deployment=" + war1.getName() + ":redeploy");
        ctx.handle("/server-group=other-server-group/deployment=" + war2.getName() + ":redeploy");
        ctx.handle("/server-group=other-server-group/deployment=" + war3.getName() + ":redeploy");

        assertEquals("OVERRIDDEN", performHttpCall("slave", "server-one", "deployment0"));
        assertEquals("NON OVERRIDDEN", performHttpCall("slave", "server-one", "deployment1"));
        assertEquals("OVERRIDDEN", performHttpCall("slave", "server-one", "another"));

        ctx.handle("deployment-overlay link --name=overlay-test --deployments=" + war2.getName() + " --redeploy-affected --server-groups=other-server-group");

        assertEquals("OVERRIDDEN", performHttpCall("slave", "server-one", "deployment0"));
        assertEquals("OVERRIDDEN", performHttpCall("slave", "server-one", "deployment1"));
        assertEquals("OVERRIDDEN", performHttpCall("slave", "server-one", "another"));

        ctx.handle("deployment-overlay remove --name=overlay-test --deployments=" + war2.getName() + " --redeploy-affected --server-groups=other-server-group");

        assertEquals("OVERRIDDEN", performHttpCall("slave", "server-one", "deployment0"));
        assertEquals("NON OVERRIDDEN", performHttpCall("slave", "server-one", "deployment1"));
        assertEquals("OVERRIDDEN", performHttpCall("slave", "server-one", "another"));

        ctx.handle("deployment-overlay remove --name=overlay-test --deployments=a*.war --server-groups=other-server-group");
    ...
    public void testRedeployAffected() throws Exception {
        ...
        ctx.handle("deployment-overlay add --name=overlay-test --content=WEB-INF/web.xml=" + overrideXml.getAbsolutePath());
        ctx.handle("deployment-overlay link --name=overlay-test --deployments=deployment0.war,a*.war --server-groups=other-server-group");
