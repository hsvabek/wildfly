...
import org.jboss.as.test.integration.domain.mixed.servlet.DeploymentOverlayServlet;
 ...
 */
public abstract class DeploymentOverlayTestCase {
    ...
    static {
        ...
        MAIN_SERVER_GROUP_ADDRESS.add(SERVER_GROUP, "other-server-group");
        MAIN_SERVER_GROUP_ADDRESS.protect();
        MAIN_SERVER_GROUP_DEPLOYMENT_ADDRESS.add(SERVER_GROUP, "other-server-group");
        MAIN_SERVER_GROUP_DEPLOYMENT_ADDRESS.add(DEPLOYMENT, TEST);
        MAIN_SERVER_GROUP_DEPLOYMENT_ADDRESS.protect();
        MAIN_RUNNING_SERVER_ADDRESS.add(HOST, "slave");
        MAIN_RUNNING_SERVER_ADDRESS.add(SERVER, "server-one");
        MAIN_RUNNING_SERVER_ADDRESS.protect();
        MAIN_RUNNING_SERVER_DEPLOYMENT_ADDRESS.add(HOST, "slave");
        MAIN_RUNNING_SERVER_DEPLOYMENT_ADDRESS.add(SERVER, "server-one");
    ...
    public static void before(Class<?> testClass) throws Exception {
        ...
        // Launch the domain
//        testSupport = MixedDomainTestSuite.createSupport(DeploymentOverlayTestCase.class.getSimpleName());
    ...
    public void setupDeploymentOverride() throws Exception {
        ...
        op = new ModelNode();
        addr = new ModelNode();
        addr.add(ModelDescriptionConstants.SERVER_GROUP, "other-server-group");
        addr.add(ModelDescriptionConstants.DEPLOYMENT_OVERLAY, TEST_OVERLAY);
        op.get(ModelDescriptionConstants.OP_ADDR).set(addr);
        op.get(ModelDescriptionConstants.OP).set(ModelDescriptionConstants.ADD);
        executeOnMaster(op);
    ...
    public void testDeploymentOverlayInDomainMode() throws Exception {
        ...
        Assert.assertEquals("OVERRIDDEN", performHttpCall(client, "slave", "server-one", "full-ha-sockets"));
