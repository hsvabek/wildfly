 ...
 */
public abstract class JmsCliTestCase extends AbstractCliTestBase {

    private static String profileName;

    public static void before(Class<?> testClass) throws Exception {
        initSuite();
//        support = MixedDomainTestSuite.getSupport(testClass, "master-config/domain.xml", true);
    ...
    @Before
    public void init() {
        profileName = serverProfiles.keySet().iterator().next();
    }
...
//        cli.sendLine("help --commands");
//        cli.sendLine("/host=slave/server=server-one:read-children-names(child-type=subsystem)");
//        ls = cli.readOutput();
//        cli.sendLine("jms-queue --profile=full-ha --help add");
//        if(true){
//            throw new IllegalStateException(ls);
//        }
    ...
    private void testAddJmsTopic() throws Exception {
        ...
        // check it is listed
        cli.sendLine("cd /profile=" + profileName + "/subsystem=messaging/hornetq-hornetq-server=default/jms-topic");
        cli.sendLine("ls");
        ls = cli.readOutput();
    ...
    private void testRemoveJmsTopic() throws Exception {
        ...
        // check it is listed
        cli.sendLine("cd /profile=" + profileName + "/subsystem=messaging/hornetq/jms-topic");
        cli.sendLine("ls");
        String ls = cli.readOutput();
        assertFalse(ls.contains("testJmsTopic"));
    }
